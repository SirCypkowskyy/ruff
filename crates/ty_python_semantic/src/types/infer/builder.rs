...
impl<'db, 'ast> TypeInferenceBuilder<'db, 'ast> {
    ...
    fn infer_match_pattern_definition(
        ...
        let binding_ty = self
            .match_pattern_binding_ty(pattern, subject_ty, index, &mut current_index)
        ...
        let found = match pattern {
            ...
            ast::Pattern::MatchMapping(match_mapping) => {
                ...
                let found = match_mapping.patterns.iter().find_map(|pattern| {
                    self.match_pattern_binding_ty(pattern, value_ty, target_index, current_index)
                });

                found.or_else(|| {
                    if match_mapping.rest.is_some() && *current_index == target_index {
                        Some(subject_ty)
                    } else {
                        None
                    }
            ...
            ast::Pattern::MatchStar(match_star) => {
                if match_star.name.is_some() && *current_index == target_index {
                    Some(subject_ty)
                } else {
                    None
                }
            ...
            ast::Pattern::MatchAs(match_as) => {
                let found = match_as.pattern.as_deref().and_then(|pattern| {
                    self.match_pattern_binding_ty(pattern, subject_ty, target_index, current_index)
                });

                found.or_else(|| {
                    if match_as.name.is_some() && *current_index == target_index {
                        Some(
                            match_as
                                .pattern
                                .as_deref()
